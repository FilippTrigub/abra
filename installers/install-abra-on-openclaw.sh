...
select_enabled_skills() {
    ...
    if [ "${env_val}" = "all" ]; then
        ...
        PROVIDER_ENABLED_MIXPANEL=1; PROVIDER_ENABLED_AMPLITUDE=1; PROVIDER_ENABLED_HOTJAR=1; PROVIDER_ENABLED_OPTIMIZELY=1; PROVIDER_ENABLED_POSTHOG=1
    ...
    if [ -n "${env_val}" ]; then
        ...
        for skill in "${skills_list[@]}"; do
            ...
            case "${skill}" in
                ...
                funnel-optimizer) SKILL_ENABLED_FUNNEL_OPTIMIZER=1; PROVIDER_ENABLED_MIXPANEL=1; PROVIDER_ENABLED_AMPLITUDE=1; PROVIDER_ENABLED_HOTJAR=1; PROVIDER_ENABLED_OPTIMIZELY=1; PROVIDER_ENABLED_POSTHOG=1 ;;
    ...
    if [ -t 0 ]; then
        ...
        if read -r -p "Enable funnel-optimizer (analytics)? [${default}]: " reply; [ "${reply:-${default}}" = "y" ]; then
            SKILL_ENABLED_FUNNEL_OPTIMIZER=1
            echo "  Analytics providers: Mixpanel, Amplitude, Hotjar, Optimizely, PostHog (need at least one)"
            read -r -p "  Enter providers (comma-separated, or 'all') [all]: " providers
            if [ -z "${providers}" ] || [ "${providers}" = "all" ]; then
                PROVIDER_ENABLED_MIXPANEL=1; PROVIDER_ENABLED_AMPLITUDE=1; PROVIDER_ENABLED_HOTJAR=1; PROVIDER_ENABLED_OPTIMIZELY=1; PROVIDER_ENABLED_POSTHOG=1
            else
                ...
                for p in "${prov_list[@]}"; do
                    ...
                    case "${p}" in
                        ...
                        posthog) PROVIDER_ENABLED_POSTHOG=1 ;;
                    esac
                done
            fi
        fi
