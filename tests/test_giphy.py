"""
test_giphy.py — Tests for the giphy skill (GIPHY-sourced GIF overlays + optional SFX).
...
Key differences from sticker:
  - GIPHY_API_KEY is the only external API (primary).
  - freesound: prefix in resolve_sfx() raises a skill-routing error (not API error).
"""
...
# ---------------------------------------------------------------------------
# Module isolation — each test gets a clean slate so skills don't shadow each other
# ---------------------------------------------------------------------------
...
class TestConfig:
    ...
    def test_validate_config_accepts_empty_effects(self) -> None:
        from giphy import validate_config
    ...
    def test_apply_preset_merges_defaults(self) -> None:
        from giphy import apply_preset
...
class TestAssetResolution:
    ...
    def test_resolve_freesound_prefix_raises_skill_redirect(
        self, tmp_path: Path
    ) -> None:
        """freesound: is not supported in the giphy skill — should redirect to skills/freesound."""
        from assets import resolve_sfx
...
# ---------------------------------------------------------------------------
# TestGiphyIntegration — runs actual FFmpeg pipeline
# ---------------------------------------------------------------------------


class TestGiphyIntegration:
    """Integration tests: full video processing via giphy.py CLI."""
    ...
    def test_exits_zero_on_success(
        ...
        result = run_skill(self.SKILL, "giphy.py", ["--config", str(cfg)])
    ...
    def test_no_effects_copies_video(
        ...
        cfg = self._cfg(tmp_path, workdir, [])
        run_skill(self.SKILL, "giphy.py", ["--config", str(cfg)])
        _, out = workdir
