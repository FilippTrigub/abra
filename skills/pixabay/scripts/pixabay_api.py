#!/usr/bin/env python3
"""pixabay_api.py — Search and download images and short videos for the pixabay skill."""
...
def _print_images(hits: list[dict]) -> None:
    if not hits:
        print("No results.")
        return
    print(f"{'#':<3} {'ID':<10} {'WxH':<14} {'Tags'}")
    print("-" * 72)
    for i, hit in enumerate(hits, 1):
        dims = f"{hit['imageWidth']}x{hit['imageHeight']}"
        tags = hit.get("tags", "")[:45]
        print(f"{i:<3} {hit['id']:<10} {dims:<14} {tags}")
...
def _print_videos(hits: list[dict]) -> None:
    if not hits:
        ...
        return
    print(f"{'#':<3} {'ID':<10} {'Dur':>5}s  {'WxH':<14} {'Tags'}")
    print("-" * 72)
    for i, hit in enumerate(hits, 1):
        med = hit["videos"].get("small") or hit["videos"].get("tiny", {})
        dims = f"{med.get('width', '?')}x{med.get('height', '?')}"
        tags = hit.get("tags", "")[:40]
        print(f"{i:<3} {hit['id']:<10} {hit['duration']:>5}s  {dims:<14} {tags}")
...
def cmd_images(args: argparse.Namespace) -> None:
    ...
    if args.add_to_library:
        lib_path = library_add(name, path, SKILL_DIR, kind="image")
...
def cmd_videos(args: argparse.Namespace) -> None:
    ...
    if not hits:
        print(
            f"No results <= {args.max_duration}s - nothing downloaded.",
            file=sys.stderr,
        )
        sys.exit(1)
...
def main() -> None:
    ...
    p_img.add_argument("--limit", "-n", type=int, default=5, help="Max results")
    p_img.add_argument("--name", help="Save as this name (stem)")
    p_img.add_argument("--pick", metavar="N", help="Download result #N")
    p_img.add_argument(
        "--image-type",
        default="all",
    ...
    p_img.add_argument("--list", action="store_true", help="List results only")
    p_img.add_argument(
        "--add-to-library",
        action="store_true",
        help="Also copy to assets/images/library/ so it can be used as local:<name>",
    )
    ...
    p_vid.add_argument("--limit", "-n", type=int, default=5, help="Max results")
    p_vid.add_argument("--name", help="Save as this name (stem)")
    p_vid.add_argument("--pick", metavar="N", help="Download result #N")
    p_vid.add_argument(
        "--max-duration",
        type=int,
        default=5,
    ...
    p_vid.add_argument("--list", action="store_true", help="List results only")
