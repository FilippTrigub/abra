"""Fetch TikTok posts and analytics for a given handle."""
...
def main():
    parser = argparse.ArgumentParser(
        description="Fetch TikTok posts and analytics for a given handle"
    )
    parser.add_argument(
        "--handle",
        required=True,
    ...
    try:
        ...
        response = client.get_tiktok_posts(
            handle=args.handle,
            max_results=args.max_results,
        )

        timestamp = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")
        output_file = output_dir / f"tiktok_{args.handle}_{timestamp}.json"
