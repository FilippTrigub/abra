"""Fetch Twitter/X posts and analytics for a given handle."""
...
def main():
    parser = argparse.ArgumentParser(
        description="Fetch Twitter/X posts and analytics for a given handle"
    )
    parser.add_argument(
        "--handle",
        required=True,
    ...
    try:
        ...
        response = client.get_twitter_posts(
            handle=args.handle,
            max_results=args.max_results,
        )

        timestamp = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")
        output_file = output_dir / f"twitter_{args.handle}_{timestamp}.json"
