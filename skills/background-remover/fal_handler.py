#!/usr/bin/env python3
"""fal.ai serverless handler for the background-remover skill (clawbg)."""
import json
import subprocess
import sys
import tempfile
...
class BackgroundRemoverApp(fal.App, keep_alive=300):
    machine_type = "GPU"
    ...
    def run(self, request: Input) -> Output:
        ...
        with tempfile.TemporaryDirectory(prefix="fal_background_remover_") as tmp_str:
            tmp = Path(tmp_str)
            input_dir = tmp / "input"
