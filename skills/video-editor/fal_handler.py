#!/usr/bin/env python3
"""fal.ai serverless handler for the video-editor skill (clawvace)."""
import json
import subprocess
import sys
import tempfile
...
class Output(BaseModel):
    output_files: list[FileResult] = Field(description="Edited output videos")


class VideoEditorApp(fal.App, keep_alive=300):
    ...
    def run(self, request: Input) -> Output:
        ...
        with tempfile.TemporaryDirectory(prefix="fal_video_editor_") as tmp_str:
            tmp = Path(tmp_str)
            input_dir = tmp / "input"
