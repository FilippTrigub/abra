#!/usr/bin/env python3
"""fal.ai serverless handler for the video-matte skill (clawmatte)."""
import json
import subprocess
import sys
import tempfile
...
class Output(BaseModel):
    output_files: list[FileResult] = Field(description="Videos with background removed")


class VideoMatteApp(fal.App, keep_alive=300):
    machine_type = "GPU"
    ...
    def run(self, request: Input) -> Output:
        ...
        with tempfile.TemporaryDirectory(prefix="fal_video_matte_") as tmp_str:
            tmp = Path(tmp_str)
            input_dir = tmp / "input"
