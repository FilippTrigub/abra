"""giphy.py — Main CLI for the giphy skill.

Overlays animated GIF stickers and social sound effects onto videos.
Supports timestamp triggers, text-cue triggers, preset effects, video pause
mode, and full-screen or positioned overlays.

Usage:
    cd skills/giphy && uv run python scripts/giphy.py
    cd skills/giphy && uv run python scripts/giphy.py --config config.json
    cd skills/giphy && uv run python scripts/giphy.py --input ./input --output ./output
"""
...
def process_video(
    ...
    """Full giphy pipeline for one video file."""
...
def process_image(
    ...
    """Apply GIF overlays to a single image."""
    output_dir.mkdir(parents=True, exist_ok=True)
    ...
    for i, eff_cfg in enumerate(config.get("effects", [])):
        ...
        if "gif" in eff_cfg:
            gif_spec = _parse_gif_spec(eff_cfg["gif"])
            resolved_gifs[i] = resolve_gif(gif_spec.source, skill_dir, favourites_path)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="giphy — Add animated GIF stickers to videos and images"
    )
    ...
    with tempfile.TemporaryDirectory(prefix="giphy_") as tmp_str:
        tmp = Path(tmp_str)
        for media_path in media_files:
            ...
            try:
                ...
                print(f"  ✗ {media_path.name}: {exc}", file=sys.stderr)
