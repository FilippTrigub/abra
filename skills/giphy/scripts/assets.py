"""assets.py — Asset resolution and favourites management for the giphy skill.

Handles bundled assets, GIPHY API, and a persistent
favourites store (favourites.json).
...
# Populate these by adding files to assets/sfx/ or assets/sfx/library/.
...
def resolve_gif(source: str, skill_dir: Path, favourites_path: Path) -> Path:
    ...
    Patterns:
      ...
      "giphy:<query>"      → GIPHY API search + cache download
    ...
    if source.startswith("bundled:"):
        ...
        if not path.exists():
            raise FileNotFoundError(
                ...
                "Run: cd skills/giphy && python scripts/generate_bundled_assets.py"
            )
        return path
    ...
    if source.startswith("giphy:"):
        query = source[len("giphy:") :]
        cache_dir = skill_dir / "assets" / "gifs" / "cache"
...
def resolve_sfx(source: str, skill_dir: Path, favourites_path: Path) -> Path:
    ...
    if source.startswith("bundled:"):
        ...
        raise FileNotFoundError(
            ...
            "Run: add bundled SFX files under skills/giphy/assets/sfx/ or use local:<name> from assets/sfx/library/"
        )
    ...
    if source.startswith("freesound:"):
        raise ValueError(
            "freesound: sources require the skills/freesound skill. "
            "Use bundled:<name>, local:<name>, or a file path instead."
        )

    if source.startswith("giphy:"):
        raise ValueError("Use bundled:<name>, local:<name>, favourite:<name>, or a file path for sound effects")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Manage giphy skill assets")
    sub = parser.add_subparsers(dest="cmd")

    sub.add_parser("list", help="List all favourites")

    p_lib = sub.add_parser(
        "library", help="Manage local library (local:name) for GIFs and local sounds"
    )
