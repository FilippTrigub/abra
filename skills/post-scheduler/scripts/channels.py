...
def main() -> None:
    ...
    p_list.add_argument(
        "--org-id", required=True, metavar="ORG_ID", help="Organization ID"
    )
    p_list.add_argument(
        "--unlocked", action="store_true", help="Return only unlocked channels"
    )

    p_get = sub.add_parser("get", help="Get a single channel by ID")
    p_get.add_argument(
        "--channel-id", required=True, metavar="CHANNEL_ID", help="Channel ID"
    )
