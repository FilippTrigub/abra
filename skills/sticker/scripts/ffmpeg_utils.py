"""ffmpeg_utils.py — FFmpeg command builders and audio mixing for the sticker skill.
...
# ---------------------------------------------------------------------------
# Data models (shared with sticker.py via import)
# ---------------------------------------------------------------------------
...
def mix_sfx_into_audio(
    ...
    if duck_background:
        ...
        bg_filter = (
            f"[0:a]volume=enable='between(t,{sfx_at_seconds:.3f},"
            f"{sfx_at_seconds:.3f}+{sfx_path})':"
            f"volume={duck_linear:.4f}[bg]"
        )
        filter_complex = (
            f"[0:a]volume=1[bg_full];"
            f"[1:a]adelay={delay_ms}|{delay_ms},volume={vol_factor:.4f}[sfx];"
            f"[bg_full][sfx]amix=inputs=2:duration=first:dropout_transition=0[out]"
        )
    else:
        filter_complex = (
            f"[1:a]adelay={delay_ms}|{delay_ms},volume={vol_factor:.4f}[sfx];"
