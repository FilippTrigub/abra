"""sticker.py — Main CLI for the sticker skill.
...
Usage:
    cd skills/sticker && uv run python scripts/sticker.py
    cd skills/sticker && uv run python scripts/sticker.py --config config.json
    cd skills/sticker && uv run python scripts/sticker.py --input ./input --output ./output
"""
...
def process_video(
    ...
    """Full sticker pipeline for one video file."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="sticker — Add animated GIF stickers and sound effects to videos"
    )
    ...
    with tempfile.TemporaryDirectory(prefix="sticker_") as tmp_str:
        tmp = Path(tmp_str)
