#!/usr/bin/env python3
"""fal.ai serverless handler for the audio-splitter skill (clawsep)."""
import json
import subprocess
import sys
import tempfile
...
_SCRIPT = Path("/app/skills/audio-splitter/scripts/separate.py")


class Input(BaseModel):
    input_urls: list[str] = Field(description="Input audio/video URLs from fal CDN")
...
class Output(BaseModel):
    output_files: list[FileResult] = Field(description="Separated audio/video files")


class AudioSplitterApp(fal.App, keep_alive=300):
    machine_type = "GPU"
    ...
    def run(self, request: Input) -> Output:
        ...
        with tempfile.TemporaryDirectory(prefix="fal_audio_splitter_") as tmp_str:
            tmp = Path(tmp_str)
            input_dir = tmp / "input"
