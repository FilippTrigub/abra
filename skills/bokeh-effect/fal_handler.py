#!/usr/bin/env python3
"""fal.ai serverless handler for the bokeh-effect skill (clawdepth)."""
import json
import subprocess
import sys
import tempfile
...
_SCRIPT = Path("/app/skills/bokeh-effect/scripts/bokeh.py")


class Input(BaseModel):
    input_urls: list[str] = Field(description="Input image URLs from fal CDN")
...
class Output(BaseModel):
    output_files: list[FileResult] = Field(description="Processed output files")


class BokehEffectApp(fal.App, keep_alive=300):
    machine_type = "GPU"
    ...
    def run(self, request: Input) -> Output:
        ...
        with tempfile.TemporaryDirectory(prefix="fal_bokeh_effect_") as tmp_str:
            tmp = Path(tmp_str)
            input_dir = tmp / "input"
