#!/usr/bin/env python3
"""fal.ai serverless handler for the photo-picker skill (clawaes)."""
import json
import subprocess
import sys
import tempfile
...
_SCRIPT = Path("/app/skills/photo-picker/scripts/score.py")
...
class Output(BaseModel):
    output_files: list[FileResult] = Field(description="Selected best images")


class PhotoPickerApp(fal.App, keep_alive=300):
    machine_type = "GPU"
    ...
    def run(self, request: Input) -> Output:
        ...
        with tempfile.TemporaryDirectory(prefix="fal_photo_picker_") as tmp_str:
            tmp = Path(tmp_str)
            input_dir = tmp / "input"
