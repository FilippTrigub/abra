...
| Init container fails with `Could not patch BaseAdapter.handle_message for Abra managed admission` | The platform-generated hydration script is patching Hermes gateway code and must match the current upstream `BaseAdapter.handle_message` signature. On 2026-06-29 the live Hermes image used `async def handle_message(self, event: MessageEvent) -> None:`, while the generated regex only matched unannotated `event):` signatures, causing `init-hydration` to crash-loop and the deployment to fail before the main container started. Fixed in `platform/src/lib/orchestration/manifest-generator.ts` by matching any `handle_message(self, event...)` signature line ending in `:`. Deploy the platform fix; no new runtime image is required for this specific failure. |
